...
assert.equal(cartItem1.specialRequest, '', "Cart item should have no special request initially");
console.log("Adding item to cart test: PASSED");

// Test adding a second bag
const removedItems = [1]; // Remove item with ID 1 (Sandwich)
const specialRequest = "Don't include any animal related products, keep it vegan";
const cartItem2 = cart.addItem(new CartItem(bag2, removedItems, specialRequest));
...
assert.equal(cartItem2.specialRequest, specialRequest, "Second cart item should have the special request");
